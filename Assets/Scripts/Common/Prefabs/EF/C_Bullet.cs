...
public class C_Bullet : MonoBehaviour
{
    ...
    public async void Move(Transform parent, Vector3 finish, C_Hero target, float timeDlMove = 0.0f, List<C_Hero> targets = null)
    {
        // Khởi tạo biến
        t = 0.0f;        
        ...
        while (true)
        {
            ...
            if (Vector3.Distance(transform.position, finish) < 0.001f || (islmH && transform.position.y >= lmH) || t > timet)
            {
                ...
                if (isComback)
                {
                    Comback(!(target.character.team == 1));
                }
                else
                {
    ...
    public void ScaleX(Vector2 A, Vector3 B)
    {
